...
*/
console.log('part 4 - 1st while loop')
let x = 0
while (x < ages.length){
	console.log(ages[x]);
	x++;
}
console.log('part 4 - 2nd while loop')
let younger2 =[]

let y = 0;
while (y < ages.length){
	if (ages[y] <= 20) {
		younger2.push(ages[y]);
	}
	y++;
}
console.log('younger 2 --- ' + younger2);

console.log('part 4 - 3rd while loop')
let z = younger.length - 1;
while (z >= 0) {
...
function pizzaTime(array){
	var pepperoniPizza = ['pepperoni'];
	for(let i = 0; i < array.length; i++){
		if (array[i].toLowerCase() !== 'pepperoni') {
			pepperoniPizza.push(array[i]);
		}
	}
	return pepperoniPizza;
}

console.log(pizzaTime(['pesto', 'sun-dried tomatoes', 'black olives', 'mushrooms']));
