...
var getDayOfWeek = function(dayInNumberForm) {
	switch(dayInNumberForm) {
		case 1:
			return "Sunday";
			break;
		case 2:
...
var addPepperoniToPizza = function(...pizzaIngridents){
	pizzaArray.push(pizzaIngridents);
	if(pizzaArray.indexOf("pepperoni") === -1){
		pizzaArray.push("pepperoni");
		console.log(`We're adding pepperoni, deal with it. Here is what your "ordered" ${pizzaArray}`);
	} else if (pizzaArray.indexOf("pepperoni") > -1){
		console.log(`Thanks for visiting Pepperoni On Every Pie, to confirm, you ordered ${pizzaArray} on your pie`);
	}
}

addPepperoniToPizza('cheese','olives','pineapple');
