...
//date comes from list above
db.Book.deleteMany({}, function(err, books){ //clears db, looks for error
  if(err) {
    ...
    db.Book.create(books_list, function(err, books){
      ...
      process.exit(); //exits the process. wanna exit in seed files
      //process above is node related
    });
  }
});
