...
while (i<ages.length) {
	i++
	if (ages[i] <= 20) {
		youngerAges.push(ages[i]);
	};
};
console.log(youngerAges);

i = younger.length;
while (i>0) {
	console.log(youngerAges[i-1]);
	i--;
};
...
*/
ingredients = ['onions', 'olives', 'pineapple'];
withPepperoni = ['sausage', 'ham', 'pineapple', 'pepperoni'];
...
console.log(addPepperoniToPizza(ingredients));
console.log(addPepperoniToPizza(withPepperoni));
