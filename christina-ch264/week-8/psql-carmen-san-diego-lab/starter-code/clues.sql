...
-- everything we are selecting from the db (like GET)
SELECT c.name, c.region, l.language
-- these are out databases
...
SELECT * FROM city y, country c
WHERE y.countrycode = c.code
...
SELECT y.name, c.name
FROM city y, country c

WHERE c.region = 'South America'
AND y.name LIKE 'Serra%'
-- connect the two db
AND y.countrycode = c.code;

-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towards
 -- the capital! Look up the country's capital, and get there pronto! Send us the name of where you're headed and we'll
 -- follow right behind you!

Select c.capital, y.name, c.name
From country c, city y
Where c.name = 'Brazil'
AND c.capital = y.id;
...
-- She's in _____________Santa Monica, California_________!
