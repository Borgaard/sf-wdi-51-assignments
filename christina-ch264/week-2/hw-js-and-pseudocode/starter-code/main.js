...
var age = 30;
var address = "Cologne";
var greeting = console.log("Hello, I'm " + name + ". I am " + age + " years old and live in " + address);
...
let names = `Ana`;
let ages = 30;
let addresss = `Barcelona`;
let greeting2 = console.log(`Hello, I'm ${names}, I am ${ages} years old and live in ${addresss}`);
...
const side1 = 5;
const side2 = 12;
const side3 = Math.hypot(Math.pow(side1, 2) + Math.pow(side2, 2));
console.log(`a=${side1}, b=${side2}, c=${side3}`);
...
variable "container",
for the length of the numbers on the list, check one number after another,
	if the first number on list is larger than the number in the container,
		put number on list in container,
	else if
		the next number on list is larger than the number in container
		put that next number on the list in the container
	return largest number in container

// For the Bonus:
// Same algorithm but checking if the number on the list is smaller than the number in the contaiener instead
