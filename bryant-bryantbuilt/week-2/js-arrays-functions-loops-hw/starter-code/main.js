...
// Part 1 as while loop
var i = 0;
while (i < ages.length) {
	console.log(ages[i]);
	i++
}
// Part 2 as while loop
while (i < ages.length) {
	if (ages[i] <= 20) {
		younger.push(ages[i]);
		i++
...
let pizzaTopping = ['pepperoni','mushrooms'];
let addToppings = function(topping) {
	if (pizzaTopping.indexOf(topping) === -1) {
		pizzaTopping.push(topping);
