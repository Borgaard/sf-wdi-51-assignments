...
let dog = new animal("scooby","dog","woof");
dog.makeNoise();
let cat = new animal("yuzu","cat","meow");
cat.makeNoise();
let bird = new animal("flappy","bird","squak");
bird.makeNoise();
let mouse = new animal("ratboi","mouse","wuddup");
mouse.makeNoise();
...
for (var i = 0; i < animals.length; i++) {
	animals[i].makeFriend = function(newFriend) {
...
for (var i = 0; i < animals.length; i++) {
	animals[i].imageLink = `images/${animals[i].type}.jpg`;
	animals[i].appear = function () {
		document.querySelector('img').setAttribute('src',this.imageLink);
	};
}
