...
*/
console.log("Part4 with while for #1 ");
var i = 0;
while (i < ages.length){
	console.log(ages[i]);
	i++;
...
console.log("Part4 with while for #2 ");
var youngers = [];
var i = 0;
while (i < ages.length){
	if (ages[i] <= 20){
			youngers.push(ages[i]);
	}
	i++;
}
console.log(youngers);

console.log("Part4 with while for #3 ");
var i = youngers.length;
while (i >= 0){
	console.log(youngers[i]);
	i--;
}
/*
...
var getDayOfWeek = function(dayy){
	switch(dayy){
		case 1 :
			console.log("Monday");
			break;
...
var addPepperoniToPizzas = function(array){
	let y = array.indexOf("pepperoni");
	if (y === -1){
...
var arr1 = ['onions', 'olives', 'pineapple'];
var arr2 = ['spinach', 'pepperoni', 'mushrooms'];
var t = 0;
var addPepperoniToPizza = function(array){
	for( let i = 0; i < array.length; i++){
		if (array[i] === "pepperoni") {
			t += 1;
		}
	}
	if (t == 0){
