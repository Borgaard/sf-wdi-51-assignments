...
const dog = {
	...
	noise: 'bark',
	makeNoise: function(){
		console.log(`${this.name} has ${this.noise}ed!`)
	}
};
...
for (let i = 0; i < animals.length; i++){
	animals[i].hunger = Math.floor(Math.random() * Math.floor(10));
}
...
for (let i = 0; i < animals.length; i++){
	animals[i].walk = function (){
		...
		return this.hunger >= 1 ? this.hunger-- : `${this.name} can't walk anymore!`;
	}
}
