...
stock.fruit.forEach(fruit => {
	masonaryFruit.append(`
			<div class="card">
                  <img id= "${fruit.id}" src=${fruit.image} class="card-img-top" alt="...">
...
stock.veggies.forEach(veggies => {
	masonaryVeggies.append(`
			<div class="card">
              <img id= "${veggies.id}" src=${veggies.image} class="card-img-top" alt="...">
...
let masonarySeafood = $('#card-columns-seafood');
	stock.seafood.forEach(seafood => {
		masonarySeafood.append(`
			<div class="card">
              <img id= "${seafood.id}" src=${seafood.image} class="card-img-top" alt="...">
...
let fruitCart = $('#cart');
	if (e.target !== e.currentTarget) {
		...
		foundItem = stock.fruit.find(ele => {
			return ele.name.toLowercase() === clickedItem.toLowercase()
		})
